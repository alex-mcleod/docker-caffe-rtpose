...
DEFINE_string(video_stream,         "",             "Use a video stream. Please also set camera to some value...as stream should be treated like webcam input.");
...
void* getFrameFromCam(void *i) {
    ...
    /* ALEX COMMENT: The loop which retrives frames and adds them to the
    input_queue is slow. At 30fps, this loop can't keep up and latency
    is introduced. */
    cv::VideoCapture cap;
    ...
    // Default to using video_stream argument
    if (!FLAGS_video_stream.empty()) {
      if (!cap.open(FLAGS_video_stream)) {
      ...
      // Otherwise, use the video file argument
    ...
    while(1) {
        ...
        // If stream has finished then OpenCV will log a message about failing to
        // read the RTMP packet header...however it will not throw an exception, and we
        // will continue on to the mnext statement.
        cap >> image_uchar_orig;

        if (image_uchar_orig.cols == 0 || image_uchar_orig.rows == 0) {
          // Assume stream has finished at this point.
          // NOTE: Have copied much of this from bottom of outer loop.
          LOG(INFO) << "Not enough cols or rows in most recent frame. Assuming that video stream is finished.";
          ...
          global.quit_threads = true;
          global.uistate.is_video_paused = true;
          LOG(INFO) << "Quitting video processing.";
          break;
        }
        ...
        if (!FLAGS_video.empty()) {
            ...
            // ALEX EDIT - This is to fix issue with JSON files getting overwritten
